...
def _norm_names(unit: dict[str, Any]) -> list[str]:
    ...
    if one and one.replace("\\", "/").casefold() not in seen:
        clean.insert(0, one)
    return clean
...
def _apply_faceid(graph: dict[str, Any], model_ref: list[Any], unit: dict[str, Any], shared_loader_ref: list[Any] | None, execution_contract: dict[str, Any]) -> tuple[list[Any], list[Any] | None, list[str]]:
    image_ref, added = _load_images(graph, unit)
    if not image_ref:
        return model_ref, shared_loader_ref, added
    if not shared_loader_ref:
        loader = _next_id(graph)
        graph[loader] = {"class_type": execution_contract.get("loader_node") or "IPAdapterUnifiedLoaderFaceID", "inputs": {"model": model_ref, "preset": execution_contract.get("preset") or unit.get("faceid_preset", "FACEID PLUS V2"), "lora_strength": unit.get("faceid_lora_strength", 0.75), "provider": unit.get("faceid_provider", "CUDA")}}
...
def apply_ip_adapter_patch(workflow: dict[str, Any], *, payload: Any, route: dict[str, Any] | None, available_nodes: Any, model_ref: list[Any] | tuple[Any, ...] | None, sampler_node_id: str | int = "5", sampler_model_input: str = "model") -> dict[str, Any]:
    ...
    added_all: list[str] = []
    shared_faceid = None
    faceid_contracts = {str(item.get("uid") or ""): item for item in (validation.get("faceid_execution_contracts") or [])}
    for unit in validation.get("active_units") or []:
        if str(unit.get("mode") or "standard") == "faceid":
            current_ref, shared_faceid, added = _apply_faceid(graph, current_ref, unit, shared_faceid, faceid_contracts.get(str(unit.get("uid") or ""), {}))
        else:
    ...
    block_metadata = ((validation.get("block") or {}).get("metadata") or {}) if isinstance(validation.get("block"), dict) else {}
    patch = {"extension_id": EXTENSION_ID, "applied": bool(added_all), "mutated": bool(added_all), "node_classes": sorted({graph[n].get("class_type") for n in added_all if n in graph}), "nodes_added": added_all, "units": len(validation.get("active_units") or []), "route": validation.get("route"), "reason": validation.get("reason"), "reference_deduplication": deepcopy(block_metadata.get("reference_deduplication") or {}), "faceid_execution_contracts": deepcopy(validation.get("faceid_execution_contracts") or [])}
    return {"workflow": graph, "model_ref": current_ref, "workflow_patch": patch, "validation": validation}
