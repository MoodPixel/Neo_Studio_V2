...
_ROUTE_PROFILES: dict[tuple[str, str], dict[str, Any]] = {
    ...
    ("krea2", "diffusion_model"): {
        **DEFAULT_PROFILE,
        "profile_id": "krea2_raw_native_control_lora",
        "family_group": "krea2",
        "rollout_state": AVAILABLE,
        "map_adapter": "krea2_control_lora",
        "inpaint_adapter": "implementation_target",
        "outpaint_adapter": "implementation_target",
        "model_dir": "loras",
        "model_label": "Krea 2 Control LoRA",
        "max_units": 1,
        "cfg_policy": "preserve_krea2_route_default",
        "negative_prompt_policy": "preserve_route_native",
        "allowed_controls": ["depth", "composition_silhouette", "canny"],
        "task_states": {
            TASK_MAP_CONTROL: {"generate": AVAILABLE},
            TASK_INPAINT_CONTROL: IMPLEMENTATION_TARGET,
            TASK_OUTPAINT_CONTROL: IMPLEMENTATION_TARGET,
        },
        "notes": [
            "Krea 2 Control Phase 1 uses the facok Krea2 Control LoRA loader/encode/apply chain for Generate only.",
            "The Control LoRA lives in models/loras and the control map is still produced by Neo/comfyui_controlnet_aux.",
            "One active Krea 2 control unit is supported; Identity Edit stacking is intentionally not enabled.",
            "Phase 6 adds Canny on Krea 2 RAW through the NK2E in-context adapter; the route profile remains family-scoped while intent dispatch chooses the adapter.",
        ],
    },
    ("krea2", "gguf"): {
        **DEFAULT_PROFILE,
        "profile_id": "krea2_raw_gguf_control_lora",
        "family_group": "krea2",
        "rollout_state": AVAILABLE,
        "map_adapter": "krea2_control_lora",
        "inpaint_adapter": "implementation_target",
        ...
        "notes": [
            "Krea 2 RAW GGUF changes only the transformer loader; the Krea2 Control LoRA adapter remains model-patcher based.",
            "The facok node resolves live module shapes for improved GGUF compatibility.",
            "Phase 6 adds RAW Canny through NK2E while keeping the GGUF transformer loader route-owned.",
        ],
    },
    ("krea2_turbo", "diffusion_model"): {
        **DEFAULT_PROFILE,
        "profile_id": "krea2_turbo_native_control_lora",
        "family_group": "krea2",
        "rollout_state": AVAILABLE,
        "map_adapter": "krea2_control_lora",
        "inpaint_adapter": "implementation_target",
        ...
        "cfg_policy": "preserve_krea2_route_default",
        "negative_prompt_policy": "preserve_route_native",
        "allowed_controls": ["depth", "composition_silhouette", "openpose"],
        "task_states": {
            TASK_MAP_CONTROL: {"generate": AVAILABLE},
            TASK_INPAINT_CONTROL: IMPLEMENTATION_TARGET,
            TASK_OUTPAINT_CONTROL: IMPLEMENTATION_TARGET,
        ...
        "notes": [
            "Krea 2 Turbo keeps Depth on the native Control LoRA adapter and Composition / Silhouette on Control Plus.",
            "Phase 5 adds OpenPose through the Ostris reference-conditioning/model-patch adapter with a model-only pose Control LoRA.",
        ],
    },
    ("krea2_turbo", "gguf"): {
        **DEFAULT_PROFILE,
        "profile_id": "krea2_turbo_gguf_control_lora",
        "family_group": "krea2",
        "rollout_state": AVAILABLE,
        "map_adapter": "krea2_control_lora",
        "inpaint_adapter": "implementation_target",
        ...
        "notes": [
            "Krea 2 Turbo GGUF preserves the same intent-specific adapter split as the native transformer route.",
            "Phase 5 OpenPose uses Ostris reference conditioning and LoraLoaderModelOnly while the base GGUF loader remains route-owned.",
        ],
    },
