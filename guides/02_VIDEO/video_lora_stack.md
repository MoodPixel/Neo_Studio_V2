...
## Phase 10 — first-class Video Assets UI

Phase 10 exposes the universal stack directly in **Video → Assets** and adds `GET /api/video/lora-catalog` for exact selected-profile catalog discovery. The endpoint validates the live `LoraLoaderModelOnly` signature before returning files and keeps filename speed classification advisory-only.

Normal user writeback now uses `extensions["video.lora_stack"]`. Legacy H3 Turbo and WAN Normal/LightX2V controls are hidden from the normal Parameters UI but remain readable through the Phase-9 compatibility boundary.

H3 Turbo/LightX2V appears as `role=speed` in the same stack on all validated H3 UNET routes. WAN dual-noise exposes semantic `all/high/low` targets. LTX primary UNET remains standard-only. No Phase-10 UI code owns workflow node IDs.

See [`video_lora_stack_ui.md`](video_lora_stack_ui.md) for the full product contract and local 33-case Phase-10 gate.
...
The existing CI-verified runtime gates preserve no-op workflow equivalence for all five H3 UNET modes, LTX UNET Txt2Vid/Img2Vid, WAN UNET Txt2Vid/Img2Vid, and WAN dual-noise GGUF Img2Vid. Phase 10 reruns those guards locally unchanged before packaging the UI layer.
