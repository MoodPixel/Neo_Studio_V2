...
Phase 21.5 keeps NeoSceneDirectorV054 as the active route and reinforces
implicit background context from the global prompt when no explicit background
region exists.
...
def _compile_appearance_lock_prompt(region: Dict, mode: str) -> str:
    ...
    # Positive conditioning only. This deliberately preserves terms already
    # written in the region prompt instead of adding fixed gender/body language.
    if mode in {"full_character_soft", "full_character_strong"}:
        parts = [
            f"full character identity and appearance authority{subject_name}",
            "subject-local full character conditioning",
            "preserve the explicitly described face, hair, skin, age, outfit, body, silhouette, proportions, and gender or presentation terms from this same region",
            "keep this same subject visually consistent inside this assigned region",
            "do not average or borrow neighboring subject traits",
            "do not swap body presentation with another subject",
            prompt,
        ]
        if mode == "full_character_strong":
            parts.insert(1, "strong full-character masked authority")
    else:
        parts = [
            f"appearance lock{subject_name}",
            "subject-local upper identity conditioning",
            "preserve the explicitly described face, hair, skin, head, shoulders, outfit cues, and presentation terms from this same region",
            "keep this same subject visually consistent inside this region",
    ...
    if mode in {"hair_focus_strong", "upper_identity_strong"}:
        parts.insert(1, "strong subject-local upper identity conditioning")
...
def _owner_display_name(region: Dict):
    ...
    prompts should speak in labels like Person 2 / Indian man so diffusion can
...
def _parse_scene_schema(
    ...
    for idx, region in enumerate(all_regions):
        ...
        if rtype == "character" and appearance_lock_mode != "off" and appearance_lock_gain > 0:
            ...
            if appearance_lock_mode in {"full_character_soft", "full_character_strong"}:
                ...
                full_mask = _make_full_character_lock_mask(mask_data, width, height, appearance_mask_weight)
                if appearance_lock_feather > 0:
                    full_mask = _feather_mask(full_mask, appearance_lock_feather)
                if appearance_lock_mode == "full_character_strong":
                    full_mask = full_mask.clamp(0, max(0.0, appearance_mask_weight * 1.10))
            ...
            if appearance_lock_mode in {"hair_focus_soft", "hair_focus_strong", "upper_identity_soft", "upper_identity_strong", "full_character_strong"}:
                ...
                upper_mask = _make_top_focus_mask(mask_data, width, height, appearance_lock_height, appearance_mask_weight)
                if appearance_lock_feather > 0:
                    upper_mask = _feather_mask(upper_mask, appearance_lock_feather)
                if upper_mode in {"hair_focus_strong", "upper_identity_strong"}:
                    upper_mask = upper_mask.clamp(0, max(0.0, appearance_mask_weight * 1.25))
    ...
    debug_json = json.dumps({
        ...
        "appearance_lock": {
            ...
            "policy": "positive subject-local full-character authority for full_character modes plus upper/head reinforcement when requested; no backend-injected negatives"
        },
...
V054_ATTACHABLE_ROLES = V054_PARENT_REQUIRED_ROLES | {
    "object", "background_object", "text", "effect", "lighting", "custom"
}
...
def _v054_background_plan_entry(region: Dict[str, Any], compiled_prompt: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "feather": _safe_int(region.get("feather", region.get("mask_feather", 32)), 32),
...
def _v054_relationship_plan_entry(parent: Dict[str, Any], child: Dict[str, Any], phrase: str, local_prompt: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "priority": _v054_priority_mode(child.get("priority")),
        "parent_prompt_injection": phrase,
...
def _v054_validate_basic(graph: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    for idx, region in enumerate(regions):
        ...
        if role in V054_PARENT_REQUIRED_ROLES and not _v054_clean_optional_id(region.get("attach_to")):
            errors.append(f"Region '{rid}' with role '{role}' requires attach_to")
        try:
    ...
    for region in regions if isinstance(regions, list) else []:
        ...
        if attach_to:
            if attach_to == rid:
                errors.append(f"Region '{rid}' cannot attach_to itself")
            if attach_to not in ids:
                errors.append(f"Region '{rid}' attach_to target '{attach_to}' was not found")
    return errors, warnings
...
def _v054_to_v05_scene(graph: Dict[str, Any], width: int, height: int, global_prompt_override: str = "") -> Tuple[str, Dict[str, Any], List[str], List[str]]:
    ...
    prompt = str(global_prompt_override or global_data.get("prompt") or "").strip()
    negative_parts = [str(global_data.get("negative", "")).strip()]
    ...
    for region in regions:
        ...
        base = {
            ...
            "feather": _safe_int(region.get("feather", region.get("mask_feather", 0)), 0),
        ...
        if role == "character":
            ...
            for child in children_by_parent.get(rid, []):
                child_role = str(child.get("role", "")).lower()
                if child_role in V054_ATTACHABLE_ROLES:
                    phrase = _v054_relationship_phrase(region, child)
                    local_prompt = _v054_local_detail_prompt(region, child)
                    if phrase:
                        parent_prompt_parts.append(phrase)
                    for conflict in _v054_conflicts_for_child(conflict_plan, str(child.get("id") or "")):
                        if conflict.get("resolution") in {"detail_override", "detail_reinforce"} and conflict.get("resolution_prompt"):
                            parent_prompt_parts.append(str(conflict.get("resolution_prompt")))
                        if conflict.get("negative_guard"):
                            negative_parts.append(str(conflict.get("negative_guard")))
                    guard = _v054_relationship_negative_guard(child)
                    if guard:
                        negative_parts.append(guard)
                    entry = _v054_relationship_plan_entry(region, child, phrase, local_prompt)
                    child_conflicts = _v054_conflicts_for_child(conflict_plan, str(child.get("id") or ""))
                    if child_conflicts:
        ...
        if role in ("held_prop", "object", "background_object"):
            ...
            if parent_region:
                ...
                local_prompt = _v054_local_detail_prompt(parent_region, region) or base["prompt"]
        ...
        if role in V054_BACKGROUND_ROLES:
            ...
            base["feather"] = _safe_int(region.get("feather", region.get("mask_feather", 32)), 32)
...
def _v054_compile_extension_authority_routes(
    ...
    for idx, raw in enumerate(raw_routes, start=1):
        ...
        try:
            ...
            feather = _safe_int(raw.get("mask_feather") or ((region.get("metadata") or {}).get("mask", {}) if isinstance(region.get("metadata"), dict) else {}).get("feather") or 12, 12)
...
def _v054_lane_prompt_for_region(region: Dict[str, Any], parent_lookup: Dict[str, Dict[str, Any]]) -> Tuple[str, str, str]:
    ...
    elif role in V054_PARENT_REQUIRED_ROLES or role in {"object", "background_object", "text", "effect", "lighting", "custom"} and _v054_clean_optional_id(region.get("attach_to")):
...
def _v054_build_region_lane_compiler(
    ...
    for index, region in enumerate(regions, start=1):
        ...
        lane = {
            ...
            "mask_feather": _safe_int(region.get("feather", region.get("mask_feather", defaults["mask_feather"])), defaults["mask_feather"]),
...
class NeoSceneDirectorV054(NeoSceneDirector):
    ...
    def INPUT_TYPES(cls):
        default_scene = json.dumps({
            "version": "v054",
            "canvas": {"width": 1344, "height": 768},
            "global": {"prompt": "cinematic rainy neon street, two young adult men standing together", "negative": "low quality, bad anatomy, extra limbs"},
            "regions": [
                {"id": "person_1", "role": "character", "label": "Person 1", "bbox": [0.05, 0.08, 0.48, 0.95], "prompt": "young Chinese man, fair skin, black office suit, red tie", "negative": "female, feminine body, breasts", "strength": 0.85, "lock": {"character": "strong", "gender": "strict", "skin_tone": "strong"}},
                {"id": "hair_1", "role": "hair_detail", "label": "Pink Spiky Hair", "attach_to": "person_1", "target_area": "hair", "relationship": "attached_to", "priority": "override", "bbox": [0.12, 0.05, 0.38, 0.28], "prompt": "vivid pink short spiky hair, natural hairline", "strength": 1.25},
                {"id": "person_2", "role": "character", "label": "Person 2", "bbox": [0.45, 0.10, 0.95, 0.98], "prompt": "young Indian man, dark brown skin, red hoodie, pink shorts, curly hair, light stubble", "negative": "female, feminine body, breasts", "strength": 0.85},
                {"id": "rose", "role": "held_prop", "label": "Rose", "attach_to": "person_2", "relationship": "holding", "priority": "reinforce", "bbox": [0.60, 0.50, 0.78, 0.78], "prompt": "single red rose flower held naturally in hand, visible stem and petals", "strength": 1.35}
            ]
        }, indent=2)
        return {
            "required": {
