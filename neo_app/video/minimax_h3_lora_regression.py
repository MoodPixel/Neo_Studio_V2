...
def synthetic_h3_object_info(*, model_only: bool = True, generic_lora: bool = False, loras: list[str] | None = None) -> dict[str, Any]:
    ...
    info["UnetLoaderGGUF"] = {
        "input": {"required": {"unet_name": _combo(["minimax_h3_fl2va-Q4_K_M.gguf", "minimax_h3_ref2va-Q4_K_M.gguf"])}},
        "output": ["MODEL"],
        "output_name": ["MODEL"],
    }
    if model_only:
...
def run_minimax_h3_lora_regression() -> dict[str, Any]:
    ...
    record(
        "H3 GGUF standard LoRA uses validated model-only path",
        lambda: _assert_profile_and_chain(_build_with_rows(_request("img2vid", loader="gguf"), standard, object_info), [STANDARD_LORA], ["standard"]),
    )
    record(
