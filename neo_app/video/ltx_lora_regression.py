...
def run_regression() -> dict[str, Any]:
    ...
    for mode in MODES:
        record(
            f"{mode}: LTX GGUF standard LoRA",
            lambda mode=mode: _assert_profile_and_chain(
                _build(mode, [{"name": STANDARD_LORA, "strength_model": 1.0, "role": "standard", "target": "all"}], object_info, loader="gguf"),
                [STANDARD_LORA],
            ),
        )
