...
def apply_ltx_model_only_lora_stack(
    ...
    *,
    ...
    if ".unet." not in route_id and ".gguf." not in route_id:
        raise ValueError("LTX 2.3 Video LoRA is fail-closed for unsupported loader topology.")
    if not loader_available:
        raise ValueError("LTX 2.3 LoRA rows were requested but ComfyUI does not expose LoraLoaderModelOnly.")
    ...
    *,
    ...
    def phase7_build(req: Any, object_info: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        profile = build_single_model_lora_patch_profile(
            ...
            loader_node_class=LTX_MODEL_ONLY_LOADER,
            validated=(".unet." in route_id or ".gguf." in route_id),
            notes=[
        ...
        runtime["route_topology"] = topology
        ...
        rules.extend(
            [
                "LTX 2.3 Txt2Vid/Img2Vid standard LoRAs use the universal Video LoRA stack and a compiler-owned model-only patch profile on UNET and validated GGUF routes.",
                "LTX speed/Turbo roles, WAN-only high/low targets, generic LoraLoader fallback, and unanchored native workflows remain blocked.",
