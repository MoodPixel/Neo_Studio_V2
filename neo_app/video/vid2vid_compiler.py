...
SCHEMA_VERSION: Final[str] = "neo.video.ltx23.vid2vid.compiler.v18"
SUPPORTED_GGUF_ROUTE_ID: Final[str] = "ltx23.gguf.vid2vid"
SUPPORTED_UNET_ROUTE_ID: Final[str] = "ltx23.unet.vid2vid"
SUPPORTED_ROUTE_IDS: Final[tuple[str, str]] = (SUPPORTED_GGUF_ROUTE_ID, SUPPORTED_UNET_ROUTE_ID)


@dataclass(frozen=True)
class LtxVid2VidCompileRequest:
    family: str = "ltx23"
    loader: str = "gguf"
    generation_type: str = "vid2vid"
    prompt: str = ""
    ...
    filename_prefix: str = "Neo_Video_LTX23_Vid2Vid"
    ...
    @classmethod
    def from_payload(cls, payload: dict[str, Any] | None) -> "LtxVid2VidCompileRequest":
        data = payload or {}
        ...
        return cls(
            ...
            generation_type=str(data.get("generation_type", data.get("mode", "vid2vid")) or "vid2vid"),
            ...
            output_format=str(data.get("output_format", "webm") or "webm"), filename_prefix=str(data.get("filename_prefix", "Neo_Video_LTX23_Vid2Vid") or "Neo_Video_LTX23_Vid2Vid"),
            ...
            denoise_strength=_float(data.get("denoise_strength", data.get("strength"))),
            motion_strength=_float(data.get("motion_strength")), frame_load_cap=_int(data.get("frame_load_cap")),
            preserve_audio=bool(data.get("preserve_audio", False)), model_name=str(data.get("model_name", "") or "") or None,
            unet_name=str(data.get("unet_name", "") or "") or None, gguf_name=str(data.get("gguf_name", "") or "") or None,
            clip_name1=str(data.get("clip_name1", data.get("clip_name", "")) or "") or None,
            clip_name2=str(data.get("clip_name2", data.get("text_projection", "")) or "") or None, vae_name=str(data.get("vae_name", "") or "") or None,
...
def resolve_vid2vid_source(req: LtxVid2VidCompileRequest) -> dict[str, Any]:
    ...
    if source_path is None:
        return {"ok": False, "error": "Video-to-Video requires a Neo-owned source video under neo_data/outputs/video.", "source_result_id": req.source_result_id or "", "source_file_id": req.source_file_id or "", "source_video_path": req.source_video_path or ""}
...
def discover_vid2vid_bindings(loader: str, object_info: dict[str, Any] | None = None) -> dict[str, Any]:
    info = object_info or {}
    ...
    classes["encode_node"] = _class_exists(info, "VAEEncodeTiled", "VAEEncode", "VAEEncodeTiledKJ", "VAEEncodeVideo") or "VAEEncodeTiled"
    available = dict(base.get("available", {}))
    available.update({"load_video": classes["load_video"] in info, "encode_node": classes["encode_node"] in info})
    return {**base, "classes": classes, "available": available, "phase": "V18"}


def _vae_encode_inputs(required: dict[str, Any]) -> dict[str, Any]:
    image_key = _field(required, ("pixels", "image", "images", "frames"), "pixels")
    inputs: dict[str, Any] = {image_key: ["0", 0], "vae": ["3", 0]}
    if "tile_size" in required:
        inputs["tile_size"] = 512
    if "overlap" in required:
        inputs["overlap"] = 64
    if "temporal_size" in required:
        inputs["temporal_size"] = 4096
    if "temporal_overlap" in required:
        inputs["temporal_overlap"] = 8
    return inputs


def build_ltx23_vid2vid_workflow(req: LtxVid2VidCompileRequest, source: dict[str, Any], object_info: dict[str, Any] | None = None) -> dict[str, Any]:
    info = object_info or {}
    loader = normalize_video_loader(req.loader)
    bindings = discover_vid2vid_bindings(loader, info)
    classes = bindings["classes"]
    params = _txt2vid_vram_safety(req)  # type: ignore[arg-type]
    denoise = _clamp01(req.denoise_strength, 0.45)
    motion = _clamp01(req.motion_strength, 0.85)
    prompt = req.prompt.strip() or "Restyle the source video while preserving its motion, timing, and camera movement."
    negative = req.negative_prompt.strip() or DEFAULT_NEGATIVE_PROMPT
    prefix = sanitize_path_part(req.filename_prefix or "Neo_Video_LTX23_Vid2Vid", fallback="Neo_Video_LTX23_Vid2Vid")

    load_video_required = _required_inputs(info, classes["load_video"])
    load_inputs = _load_video_inputs(load_video_required, source)
    if req.frame_load_cap and "frame_load_cap" in load_video_required:
        load_inputs["frame_load_cap"] = req.frame_load_cap
    encode_required = _required_inputs(info, classes["encode_node"])
    scheduler_required = _required_inputs(info, classes["scheduler_node"])
    scheduler_inputs = {"latent": ["16", 2]}
    ...
    workflow = {
        "0": {"class_type": classes["load_video"], "inputs": load_inputs},
        ...
        "7": {"class_type": classes["encode_node"], "inputs": _vae_encode_inputs(encode_required)},
        ...
        "9": {"class_type": classes["guider_node"], "inputs": {"model": ["8", 0], "positive": ["16", 0], "negative": ["16", 1], "cfg": params["guidance"]}},
        "10": {"class_type": classes["sampler_select_node"], "inputs": {"sampler_name": params["sampler"]}},
        "11": {"class_type": classes["sampler_node"], "inputs": {"noise": ["13", 0], "guider": ["9", 0], "sampler": ["10", 0], "sigmas": ["14", 0], "latent_image": ["16", 2]}},
    ...
    return {"schema_version": SCHEMA_VERSION, "surface": "video", "phase": "V18", "route_id": f"ltx23.{loader}.vid2vid", "compiled_at": _now(), "parameters": {**{k: v for k, v in params.items() if k != "profile"}, "denoise_strength": denoise, "motion_strength": motion, "frame_load_cap": req.frame_load_cap or 0, "preserve_audio": req.preserve_audio}, "profile": params["profile"], "vram_engine": params.get("vram_engine", {}), "bindings": bindings, "workflow": workflow, "prompt_api_payload": {"prompt": workflow}, "source": {**source, "required": True, "denoise_strength": denoise, "motion_strength": motion}, "rules": ["V18 Video-to-Video uses a Neo-owned source video as encoded source latents.", "The source video is never modified; output is a child result under neo_data/outputs/video/vid2vid.", "This first V2V lane targets LTX 2.3 GGUF and UNET/Diffusion routes only.", "This is a controlled restyle/cleanup bridge, not the V14 Finish repair lane."]}


def video_ltx23_vid2vid_compile_payload(payload: dict[str, Any] | None = None, object_info_override: dict[str, Any] | None = None) -> dict[str, Any]:
    req = LtxVid2VidCompileRequest.from_payload(payload)
    ...
    if not route or route.route_id not in SUPPORTED_ROUTE_IDS:
        return {"schema_version": SCHEMA_VERSION, "surface": "video", "phase": "V18", "ok": False, "queued": False, "error": f"V18 Video-to-Video compiler only supports {SUPPORTED_GGUF_ROUTE_ID} and {SUPPORTED_UNET_ROUTE_ID}.", "request": {"family": nf, "loader": nl, "generation_type": nt}, "route": route.payload() if route else None}
    source = resolve_vid2vid_source(req)
    if not source.get("ok"):
        return {"schema_version": SCHEMA_VERSION, "surface": "video", "phase": "V18", "ok": False, "queued": False, "error": source.get("error") or "Video-to-Video source video is missing.", "request": req.payload(), "source": source, "route": route.payload()}
    ...
    if object_info_override is None:
        try:
            ...
            warnings.append(f"Compiled with fallback LTX Video-to-Video bindings because ComfyUI /object_info was unavailable: {exc}")
            object_info = {}
    compiled = build_ltx23_vid2vid_workflow(req, source, object_info=object_info)
    ...
    output_paths = get_video_output_paths("vid2vid", create=True)
    metadata_dir = get_video_output_paths("metadata", create=True).output_dir
    sidecar_name = f"{sanitize_path_part(req.filename_prefix, 'ltx23_vid2vid')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_compile.json"
...
def video_ltx23_vid2vid_generate_payload(payload: dict[str, Any] | None = None, object_info_override: dict[str, Any] | None = None, timeout: float = 5.0) -> dict[str, Any]:
    req = LtxVid2VidCompileRequest.from_payload(payload)
    compile_payload = video_ltx23_vid2vid_compile_payload({**req.payload(), "dry_run": True}, object_info_override=object_info_override)
    ...
    if object_info_override is not None and not compile_payload.get("route_readiness", {}).get("ready"):
        return {**compile_payload, "ok": False, "queued": False, "error": "Selected LTX 2.3 Video-to-Video route is missing required ComfyUI nodes."}
    try:
