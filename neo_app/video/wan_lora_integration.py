...
from neo_app.video.wan_unet_speed_lora import wan_speed_lora_candidates, wan_unet_speed_advisories
...
def apply_wan_single_lora_stack(
    ...
    *,
    ...
    if any(row.get("target") != "all" for row in ordered):
    ...
    for row in normalized:
        ...
        graph[node_id] = {
            ...
            "_meta": {"title": f"Video LoRA · WAN 2.2 · {row['role']}"},
        }
        upstream = [node_id, 0]
...
def _install_single_model_wan(module: Any) -> None:
    ...
    def phase8_build(req: Any, object_info: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        speed_candidates = wan_speed_lora_candidates(live)
        ...
        runtime["speed_role_classifier_is_advisory"] = True
        runtime["warnings"] = list(dict.fromkeys([*(runtime.get("warnings") or []), *wan_unet_speed_advisories(runtime.get("applied") or [], compiled.get("parameters"))]))
        runtime["generic_lora_loader_fallback"] = False
        runtime["legacy_bridge"] = {"requested": False, "bridged_count": 0}
        runtime["phase"] = "phase_15"
        compiled["video_lora_stack"] = runtime
        rules = compiled.get("rules") if isinstance(compiled.get("rules"), list) else []
        rules.extend([
            "WAN 2.2 UNET Txt2Vid/Img2Vid standard and speed/LightX2V LoRAs use the universal compiler-owned model-only patch profile.",
            "WAN single-model routes accept target='all' only; standard rows are always applied before speed rows.",
            "LightX2V filename classification is advisory; explicit role=speed selection is accepted when the file exists in the live catalog.",
