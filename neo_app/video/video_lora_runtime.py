...
def _validate_model_only_profile(workflow: dict[str, Any], profile: dict[str, Any], route_id: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if not bool(profile.get("validated", False)):
        raise ValueError("MiniMax H3 Video LoRA patch profile has not passed its loader-topology gate.")
...
def apply_h3_model_only_lora_stack(
    ...
    *,
    ...
    if ".unet." not in route_id and ".gguf." not in route_id:
        raise ValueError("Video LoRA Stack is fail-closed for unsupported MiniMax H3 loader topology.")
