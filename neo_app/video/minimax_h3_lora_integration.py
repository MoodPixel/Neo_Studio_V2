...
from neo_app.video.h3_gguf_lora_capability import validate_h3_gguf_lora_topology
...
def install_minimax_h3_lora_integration() -> None:
    ...
    def phase5_build_workflow(req: Any, object_info: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        topology = {"loader": route_id.split(".")[1] if route_id.count(".") >= 2 else "", "validated": ".unet." in route_id, "evidence": "prior_h3_unet_regression"}
        ...
        if rows:
            topology = validate_h3_gguf_lora_topology(route_id, bindings, info)
        ...
        if ".gguf." in route_id:
            runtime["gguf_topology"] = topology
        ...
        rules.extend([
            ...
            "H3 GGUF LoRA/Turbo additionally requires live /object_info proof that the GGUF loader emits MODEL and LoraLoaderModelOnly accepts MODEL.",
        ])
