...
def run_phase8_gate() -> dict[str, Any]:
    ...
    run("WAN UNET speed/LightX2V role", lambda: (
        lambda compiled: (
            _assert_single_chain(compiled, [SPEED_LORA]),
            _assert((compiled.get("video_lora_stack") or {}).get("speed_count") == 1, "WAN UNET speed row was not counted"),
            {"speed_count": 1},
        )[2]
    )(_build_single("txt2vid", [{"name": SPEED_LORA, "role": "speed", "target": "all"}], single_info)))
