...
def _seed(params: Mapping[str, Any]) -> tuple[int, int]:
    requested = int(_param(params, "requested_seed", "seed", default=-1) or -1)
    actual = int(_param(params, "actual_seed", "seed", default=requested) or requested)
    if actual < 0:
...
def compile_anima_image(*, provider_id: str, base_url: str, job: NeoJob, validation: ProviderValidationResult, route: CompileRoute, capabilities: dict[str, Any], backend_capabilities: dict[str, Any] | None = None) -> CompiledJob:
    ...
    steps = max(1, int(_param(params, "steps", default=30) or 30))
    cfg = float(_param(params, "cfg", default=4.0) or 4.0)
    denoise = float(_param(params, "denoise", default=0.75 if mode == "img2img" else 1.0) or 1.0)
    width = max(64, int(_param(params, "width", default=1024) or 1024))
    height = max(64, int(_param(params, "height", default=1024) or 1024))
...
def compile_ideogram4_txt2img(*, provider_id: str, base_url: str, job: NeoJob, validation: ProviderValidationResult, route: CompileRoute, capabilities: dict[str, Any], backend_capabilities: dict[str, Any] | None = None) -> CompiledJob:
    ...
    _require_nodes(validation,backend,required)
    requested_seed,seed=_seed(params); width=max(64,int(_param(params,"width",default=1024) or 1024)); height=max(64,int(_param(params,"height",default=1024) or 1024)); steps=max(1,int(_param(params,"steps",default=20) or 20)); cfg=float(_param(params,"cfg",default=4.0) or 4.0)
    ...
    if validation.ok:
        workflow={
          ...
          "10":{"class_type":"Ideogram4Scheduler","inputs":{"steps":steps,"width":width,"height":height,"mu":float(_param(params,"ideogram4_mu",default=0.5) or 0.5),"std":float(_param(params,"ideogram4_std",default=1.75) or 1.75)}},
