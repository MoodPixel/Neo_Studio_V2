...
def discover_comfy_backend_capabilities(
    ...
    *,
    ...
    diffusion_roles = {
        ...
        "qwen21_text_encoder": _role_requiring_option("qwen21_text_encoder", ["CLIPLoader"], object_info, option_input="type", required_option="qwen_image", asset_inputs=["clip_name", "clip_name1"], notes=["Qwen Image 2.1 Qwen3-VL 8B asset lane; filename is user-selected while architecture support is verified by CLIPLoader(type=qwen_image)."]),
        "qwen21_vae": _role("qwen21_vae", ["VAELoader"], object_info, asset_inputs=["vae_name"], notes=["Qwen Image 2.1 VAE asset lane."]),
        "qwen21_diffusion_loader": _role_requiring_inputs("qwen21_diffusion_loader", ["UNETLoader"], object_info, required_inputs=["unet_name"], asset_inputs=["unet_name"], notes=["Q21-1/Q21-2 native Safetensors/components diffusion loader."]),
        "qwen21_text_encoder_loader": _role_requiring_option("qwen21_text_encoder_loader", ["CLIPLoader"], object_info, option_input="type", required_option="qwen_image", asset_inputs=["clip_name", "clip_name1"], notes=["Q21-1/Q21-2 require CLIPLoader(type=qwen_image)."]),
        "qwen21_vae_loader": _role_requiring_inputs("qwen21_vae_loader", ["VAELoader"], object_info, required_inputs=["vae_name"], asset_inputs=["vae_name"]),
        "qwen21_conditioning": _role_requiring_inputs("qwen21_conditioning", ["TextEncodeQwenImage21"], object_info, required_inputs=["clip", "prompt", "negative_prompt", "vae", "resolution"], notes=["Q21-1 uses TextEncodeQwenImage21 for positive/negative T2I conditioning; Q21-2 also uses its ordered autogrow image inputs and latent output for unified edit."]),
        "qwen21_empty_latent": _role_requiring_inputs("qwen21_empty_latent", ["EmptyLatentImage"], object_info, required_inputs=["width", "height", "batch_size"], notes=["Matches the current official Qwen Image 2.1 T2I template latent ownership."]),
        "qwen21_sampler": _role_requiring_inputs("qwen21_sampler", ["KSampler"], object_info, required_inputs=["model", "positive", "negative", "latent_image", "seed", "steps", "cfg", "sampler_name", "scheduler", "denoise"]),
        "qwen21_decode": _role_requiring_inputs("qwen21_decode", ["VAEDecode"], object_info, required_inputs=["samples", "vae"]),
        "qwen21_alpha_split": _role_requiring_inputs("qwen21_alpha_split", ["SplitImageWithAlpha"], object_info, required_inputs=["image"], notes=["Q21-6A explicit RGB output and RGB-only model-upscaler boundaries use SplitImageWithAlpha."]),
        "qwen21_alpha_join": _role_requiring_inputs("qwen21_alpha_join", ["JoinImageWithAlpha"], object_info, required_inputs=["image", "alpha"], notes=["Q21-6A explicit RGBA edit reconstructs LoadImage alpha before Qwen VAE reference encoding."]),
        "qwen21_cache": _role_requiring_inputs("qwen21_cache", ["QwenImage21Cache"], object_info, required_inputs=["model", "device", "dtype"], notes=["Q21-6B optional Qwen Image 2.1 inference cache control with model/device/dtype sockets."]),
        "qwen21_mask_loader": _role_requiring_inputs("qwen21_mask_loader", ["LoadImageMask"], object_info, required_inputs=["image", "channel"], notes=["Q21-4 core inpaint mask loader."]),
        "qwen21_mask_grow": _role_requiring_inputs("qwen21_mask_grow", ["GrowMask"], object_info, required_inputs=["mask", "expand", "tapered_corners"], notes=["Q21-4 uses Comfy core GrowMask; no KJNodes dependency."]),
        "qwen21_outpaint_pad": _role_requiring_inputs("qwen21_outpaint_pad", ["ImagePadForOutpaint"], object_info, required_inputs=["image", "left", "top", "right", "bottom", "feathering"]),
        "qwen21_vae_encode": _role_requiring_inputs("qwen21_vae_encode", ["VAEEncode"], object_info, required_inputs=["pixels", "vae"]),
        "qwen21_latent_mask": _role_requiring_inputs("qwen21_latent_mask", ["SetLatentNoiseMask"], object_info, required_inputs=["samples", "mask"]),
        "qwen21_differential": _role_requiring_inputs("qwen21_differential", ["DifferentialDiffusion"], object_info, required_inputs=["model"]),
        "qwen21_composite": _role_requiring_inputs("qwen21_composite", ["ImageCompositeMasked"], object_info, required_inputs=["destination", "source", "x", "y", "resize_source"]),
        "qwen21_invert_mask": _role_requiring_inputs("qwen21_invert_mask", ["InvertMask"], object_info, required_inputs=["mask"]),
        ...
        "krea2_identity_lora_loader": _role("krea2_identity_lora_loader", ["LoraLoaderModelOnly"], object_info, asset_inputs=["lora_name"], notes=["Used when Krea 2 Identity Edit weight source is Separate LoRA; baked-in-model mode does not require this engine-owned loader."]),
        ...
        "krea2_ostris_text_encode": _role_requiring_inputs("krea2_ostris_text_encode", ["TextEncodeKrea2OstrisEdit"], object_info, required_inputs=["clip", "prompt", "vae", "image1"], notes=["Krea 2 Ostris Edit prompt encoder; Neo requires the primary image-edit sockets clip/prompt/vae/image1 and verifies image2/image3 only when those extra references are requested."]),
        "krea2_ostris_model_patch": _role_requiring_inputs("krea2_ostris_model_patch", ["Krea2OstrisEditModelPatch"], object_info, required_inputs=["model", "kv_cache"], notes=["Krea 2 Ostris Edit model patch node."]),
        "krea2_ostris_lora_loader": _role("krea2_ostris_lora_loader", ["LoraLoaderModelOnly"], object_info, asset_inputs=["lora_name"], notes=["Used when Krea 2 Ostris Edit weight source is Separate LoRA; baked-in-model mode does not require this engine-owned loader."]),
    ...
    gguf_roles = {
        ...
        "qwen21_gguf_diffusion_loader": _role(
            "qwen21_gguf_diffusion_loader",
            ["UnetLoaderGGUF", "LoaderGGUF"],
            object_info,
            asset_inputs=["unet_name", "model_name", "gguf_name"],
            notes=["Q21-3 GGUF diffusion-transformer loader. Neo accepts UnetLoaderGGUF(unet_name) or LoaderGGUF(gguf_name) when advertised live."],
        ),
        "qwen21_text_encoder": _role_requiring_option(
            "qwen21_text_encoder", ["CLIPLoader"], object_info,
            option_input="type", required_option="qwen_image",
            asset_inputs=["clip_name", "clip_name1"],
            notes=["Q21-3 deliberately keeps Qwen3-VL 8B native/safetensors while only the diffusion transformer is GGUF."],
        ),
        "qwen21_vae": _role("qwen21_vae", ["VAELoader"], object_info, asset_inputs=["vae_name"], notes=["Q21-3 keeps the Qwen Image 2.1 VAE native/safetensors."]),
        "qwen21_text_encoder_loader": _role_requiring_option(
            "qwen21_text_encoder_loader", ["CLIPLoader"], object_info,
            option_input="type", required_option="qwen_image",
            asset_inputs=["clip_name", "clip_name1"],
            notes=["Q21-3 mixed GGUF route still requires native CLIPLoader(type=qwen_image)."],
        ),
        "qwen21_vae_loader": _role_requiring_inputs("qwen21_vae_loader", ["VAELoader"], object_info, required_inputs=["vae_name"], asset_inputs=["vae_name"]),
        "qwen21_conditioning": _role_requiring_inputs("qwen21_conditioning", ["TextEncodeQwenImage21"], object_info, required_inputs=["clip", "prompt", "negative_prompt", "vae", "resolution"], notes=["Q21-3 reuses native TextEncodeQwenImage21 for T2I and ordered-reference edit conditioning."]),
        "qwen21_empty_latent": _role_requiring_inputs("qwen21_empty_latent", ["EmptyLatentImage"], object_info, required_inputs=["width", "height", "batch_size"]),
        "qwen21_sampler": _role_requiring_inputs("qwen21_sampler", ["KSampler"], object_info, required_inputs=["model", "positive", "negative", "latent_image", "seed", "steps", "cfg", "sampler_name", "scheduler", "denoise"]),
        "qwen21_decode": _role_requiring_inputs("qwen21_decode", ["VAEDecode"], object_info, required_inputs=["samples", "vae"]),
        "qwen21_alpha_split": _role_requiring_inputs("qwen21_alpha_split", ["SplitImageWithAlpha"], object_info, required_inputs=["image"], notes=["Q21-6A explicit RGB output and RGB-only model-upscaler boundaries use SplitImageWithAlpha."]),
        "qwen21_alpha_join": _role_requiring_inputs("qwen21_alpha_join", ["JoinImageWithAlpha"], object_info, required_inputs=["image", "alpha"], notes=["Q21-6A explicit RGBA edit reconstructs LoadImage alpha before Qwen VAE reference encoding."]),
        ...
        "krea2_identity_lora_loader": _role("krea2_identity_lora_loader", ["LoraLoaderModelOnly"], object_info, asset_inputs=["lora_name"], notes=["Used only for Separate LoRA Identity Edit. Baked-in-model mode skips this loader even when the selected Krea 2 transformer is GGUF."]),
        ...
        "krea2_ostris_text_encode": _role_requiring_inputs("krea2_ostris_text_encode", ["TextEncodeKrea2OstrisEdit"], object_info, required_inputs=["clip", "prompt", "vae", "image1"], notes=["Krea 2 Ostris Edit prompt encoder; Neo requires the primary image-edit sockets clip/prompt/vae/image1 and verifies image2/image3 only when those extra references are requested."]),
        "krea2_ostris_model_patch": _role_requiring_inputs("krea2_ostris_model_patch", ["Krea2OstrisEditModelPatch"], object_info, required_inputs=["model", "kv_cache"], notes=["Krea 2 Ostris Edit model patch node after native or GGUF Krea 2 loading."]),
        "krea2_ostris_lora_loader": _role("krea2_ostris_lora_loader", ["LoraLoaderModelOnly"], object_info, asset_inputs=["lora_name"], notes=["Used when Krea 2 Ostris Edit weight source is Separate LoRA; baked-in-model mode skips this loader even when the selected Krea 2 transformer is GGUF."]),
